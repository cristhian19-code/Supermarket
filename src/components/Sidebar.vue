<template>
    <div id="sidebar">
        <button class="menu__sidebar"><span>&#9776;</span></button>
        <div class="header__sidebar">
            <img src="https://images.pexels.com/photos/821718/pexels-photo-821718.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940" alt="" height="70px" width="70px">
        ...
        <div class="inputs__sidebar">
            <router-link class="btn__sidebar" v-for="item in sidebaritems" :key="item.name" :to="{name: item.name}">
                <i :class="item.icon"></i>{{item.name}}<span class="length__cart" v-if="item.name == 'Carrito'">{{getLengthShoppingCart}}</span>
            </router-link>
